...
static void *lcd_logo(const char *file_path);
...
void lcd_clear(const char * file_path)
{
	...
	lcd_console_address = lcd_logo(file_path);
#else
	if (lcd_show_logo)
	{
		debug("[LCD] Drawing the logo...\n");
...
#include <resource.h>
int rk_bitmap_from_resource(unsigned short* fb,const char* file_path ) 
{
...
void bitmap_plot(int x, int y,const char *file_path)
{
...
#ifdef CONFIG_RK_FB
	fb = (uchar *)(lcd_base);
	if(!rk_bitmap_from_resource((unsigned short*)fb,file_path))
		return;
#else
...
#else
static inline void bitmap_plot(int x, int y,const char *file_path) {}
...
#ifndef CONFIG_ROCKCHIP
	bitmap_plot(0, 0,file_path);
#else
	bitmap_plot((panel_info.vl_col - BMP_LOGO_WIDTH)/2, (panel_info.vl_row - BMP_LOGO_HEIGHT)/2,file_path);
#endif
